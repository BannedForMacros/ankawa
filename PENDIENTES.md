...
> - ✅ **`ArbitrajeForm.jsx` + `ArbitrajeEmergenciaForm.jsx`** — migrados (núcleo seguro). `arbitrajeSchema`/`emergenciaSchema` (Emergencia = subconjunto, sin los 4 montos de controversia) alimentados por `datosValidables()`; `validarZod` reemplaza el armado manual de `missingFields` (claves idénticas → errores inline iguales) + `confirmar()` de swalAnkawa reemplaza `ConfirmModal`. Código muerto eliminado (`mostrarError`). Build OK.
>   - **Decisión de riesgo (estos 2 forms):** se **conservó** el modal-resumen `showErrorModal` (alimentado por Zod) y el `useEffect` espejo (clear-on-change), y se **difirió el `onBlur` por campo**. Motivo: son el camino legal más crítico, no verificable en este entorno, y el `onBlur` exige enhebrar el `BloquePersona` **compartido** (multi-tipo, #2). Pendiente opcional tras verificación en navegador: quitar `showErrorModal` + espejo y enhebrar `onBlur`/clear vía `BloquePersona` (como en JPRD).
