...
> - ✅ **`ArbitrajeForm.jsx` + `ArbitrajeEmergenciaForm.jsx`** — migrados. `arbitrajeSchema`/`emergenciaSchema` (Emergencia = subconjunto, sin los 4 montos de controversia) alimentados por `datosValidables()`; `validarZod` reemplaza el armado manual de `missingFields` (claves idénticas → errores inline iguales) + `confirmar()` de swalAnkawa reemplaza `ConfirmModal`. Código muerto eliminado (`mostrarError`).
>   - **`onBlur` completado en `BloquePersona`** (jun 2026): se agregó prop `onBlurCampo` + `onBlur` en `documento`/`nombre`/`domicilio`. Como `BloquePersona` es **compartido** (exportado por Arbitraje, importado por Emergencia), el cambio cubre **ambos** forms; cada padre pasa `onBlurCampo={campo => validarBlur(\`${campo}_demandante|_demandado\`)}` (sufijo por rol) usando `validarCampo`. El `onBlur` solo dispara validación (no toca la lógica de lookup/domicilio de #2). `documento_demandado` no es requerido → su blur es no-op inofensivo.
>   - Se **conservó** a propósito: el modal-resumen `showErrorModal` (alimentado por Zod, útil para ver todo lo faltante de un vistazo en un form largo) y el `useEffect` espejo (clear-on-change). Quitarlos es opcional y no aporta valor claro.
>   - **Pendiente opcional:** `onBlur` en los campos de consorcio (`PanelConsorcio`: rep DNI/nombre, empresas) y en los de nivel padre (teléfono, montos, emails) — hoy validan al submit + clear-on-change (espejo).
> - **Falta verificación en navegador** de los 4 forms (login OTP).
